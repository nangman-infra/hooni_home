...
export function TechStack() {
    return (
        <section id="skills" className="py-12 md:py-20 border-b border-border/40">
            <Container>
                <h2 className="mb-8 md:mb-12 text-sm font-bold text-muted-foreground uppercase tracking-widest text-center">Core Focus</h2>
                    ...
                    {skillCategories.map((cat) => (
                        <div key={cat.name} className="flex flex-col items-center text-center p-6 rounded-2xl bg-secondary/5 border border-border/50 hover:bg-secondary/10 transition-colors duration-300">
                            ...
                            <div className="flex flex-wrap justify-center gap-3">
                                {cat.items.map((item) => (
                                    <span key={item} className="px-4 py-2 rounded-lg bg-background border border-border/60 text-foreground/80 text-sm font-medium text-center leading-snug shadow-sm hover:text-foreground hover:border-primary/50 transition-colors cursor-default">
                                        {item}
                                    </span>
                                ))}
                            </div>
